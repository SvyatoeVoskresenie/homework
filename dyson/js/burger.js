export default class BurgerMenu {
	constructor(config, headerFixedInstance = null) {
		...
		this.burgerMenu = document.querySelector(`.${this.config.HEADER_NAV}`);
	...
	onBurgerClick() {
		...
		this.burgerMenu.classList.toggle(this.config.HEADER_NAV_OPEN, isOpen);
	...
	hideBurgerMenu() {
		...
		this.burgerMenu.classList.remove(this.config.HEADER_NAV_OPEN);
	...
	isBurgerMenuOpen() {
		return this.burgerMenu.classList.contains(this.config.HEADER_NAV_OPEN);
	}

	onBodyClick(event) {
		const target = event.target;
		const isLinkInMenu = target.classList.contains(this.config.HEADER_NAV_LINK);
		const isMenuOpen = this.isBurgerMenuOpen();
		const isClickOutsideMenu =
			!target.closest(`.${this.config.HEADER_NAV}`) &&
